...
/***********************************
Assignment-2 -- REFACTOR 5 CARDS:  first definition of new functions
**************************************/
...
void cardIsFeast(int i, struct gameState *state, int currentPlayer, int temphand[MAX_HAND], int x, int choice2){
	...
	while (x == 1) {//Buy one card
		if (supplyCount(choice2, state) <= 0){
		...
		else if (state->coins < getCost(choice2)){
			...
			gainCard(choice2, state, 0, currentPlayer);//Gain the card
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
		cardIsAdventurer(drawntreasure, state, currentPlayer, cardDrawn, z, temphand);
		return 0;
     			
    case council_room:
		cardIsCouncilRoom(i, currentPlayer, state, handPos);
		return 0;
					
    case feast:
		cardIsFeast(i, state, currentPlayer, temphand, x, choice2);
		return 0;
    ...
    case smithy:
		cardIsSmithy(i, currentPlayer, state, handPos);
		return 0;
      		
    case village:
		cardIsVillage(currentPlayer, state, handPos);
		return 0;		
